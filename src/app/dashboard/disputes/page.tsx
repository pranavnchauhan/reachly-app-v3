...
export default function DisputesPage() {
  ...
  const [revealedLeads, setRevealedLeads] = useState<RevealedLead[]>([]);
  ...
  async function handleScreenshotUpload(channel: string, file: File) {
    ...
    const path = `${userId}/${selectedLeadId}/${channel}-${Date.now()}.${ext}`;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setEvidence({});
      // Reload
      const { data } = await supabase.from("disputes").select("*, leads(company_name, contact_name)").eq("client_id", userId).order("created_at", { ascending: false });
      setDisputes(data ?? []);
    } else {
